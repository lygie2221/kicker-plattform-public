    ...
    $.widget( 'evol.structFilter', {
        ...
        _create: function(){
            ...
            */
            ...
            // dt button new
            this._bNew=$('body').find('.dt-evo-add').on('click', function(e){
                if(that._step<1){
                    that._setEditorField();
                    that._step=1;
                }
            ...
            this._editor=e.find('.evo-editFilter')
                .on('change', '#field', function(evt){
                    evt.stopPropagation();
                    if(that._step>2){
                        //try { that._editor.find('#operator').select2('destroy'); } catch(e) {};
                        try { that._editor.find('#value,#value2').select2('destroy'); } catch(e) {};
            ...
            }).on('click', 'span.ui-icon-close', function(evt){
                evt.stopPropagation();
        ...
        _removeEditor: function(){
            ...
            this._bNew.removeClass('ui-state-active').show().removeAttr('disabled');
            if(this._bSubmit){
        ...
        _htmlFilter: function(filter){
            ...
            h+='<i class="filter-node-type" data-type="'+filter.field.label+'"></i>'
            return h;
        },
        ...
        _setEditorField: function(fid){
            if(this._step<1){
                this._bNew.stop().attr('disabled', 'DISABLED');
                if(this._bSubmit){
                    ...
                    if(this._step<3){
                        ...
                        switch (fType){
                            case fTypes.bool:
                                ...
                                break;
                            case fTypes.list:
                                h+='<select multiple="multiple" id="value">';
                        ...
                        if(fType==fTypes.listDropdown){
                            editor.find('#value,#value2').select2({width: '250px'});
                            editor.find('#value,#value2').trigger('change');

                        }
                        if(fType==fTypes.list){ // Select2
                            editor.find('#value,#value2').select2({width: '250px'});
                        }
                    }
                    if(v){
                        ...
                        switch (fType){
                            case fTypes.list:
                                var vals = v.split(',');
                                $('#value').select2('val', vals);
                                /*for(var i = 0; i < vals.length; i++) {
                                    $('#value').val(vals[i]).trigger('change');
                                }*/
                                //$value.find('#value'+v).prop('selected', 'SELECTED');
        ...
        _getEditorData: function(){
            ...
            if(this._type==fTypes.list){
                ...
                var s2data = $('#value').select2('data');
                for(var i = 0; i < s2data.length; i++) {
                    vs.push(s2data[i].id);
                    ls.push(s2data[i].text);
                }
                if(vs.length===0){
